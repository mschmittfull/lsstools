...
def highlevel_get_interp_filters_minimizing_sqerror(sources=None,
                                                    target=None,
                                                    Pk=None,
                                                    interp_kind=None,
                                                    bounds_error=False,
    ...
    (kvec, sources_X_target, sources_rms,
     sources_correl) = construct_matrices_needed_to_get_min_sqerror_trf_fcns(
    ...
    if Pktype not in [MeasuredPower1D, MeasuredPower2D]:
        raise Exception(
            'Pk must be MeasuredPower1D or MeasuredPower2D. Got %s' % Pktype)
    ...
    interp_trf_fcn_tuple = get_interp_filters_minimizing_sqerror(
        kvec,
        sources_X_target=sources_X_target,
        sources_rms=sources_rms,
    ...
    for isource, source in enumerate(sources):
        for isource2, source2 in enumerate(sources):
            if isource2 == isource:
                ...
                sources_correl[isource, isource2, :] = np.ones(Nk,
                                                               dtype='float')
            else:
                # off-diagonal correl
...
def get_filters_minimizing_sqerror_at_fixed_k(sources_X_target_vec=None,
                                              sources_rms_vec=None,
                                              sources_correl_mat=None):
    """
    ...
    return (inv_sources_rms_vec * np.linalg.solve(
        sources_correl_mat, sources_X_target_vec * inv_sources_rms_vec))


def get_filters_minimizing_sqerror(sources_X_target=None,
                                   sources_rms=None,
                                   sources_correl=None):
    """
...
def get_interp_filters_minimizing_sqerror(kvec=None,
                                          sources_X_target=None,
                                          sources_rms=None,
                                          sources_correl=None,
                                          interp_kind=None,
                                          bounds_error=None,
    ...
    for isource in range(Nsources):
        ...
        if interp_kind in ['manual_Pk_k_bins', 'manual_Pk_k_mu_bins']:
            interp_t_list[
                isource] = interpolation_utils.interp1d_manual_k_binning(
                    kvec,
                    this_tk,
                    kind=interp_kind,
            ...
            raise Exception(
                "TODO: better use interp_kind=manual_Pk_k_bins or manual_Pk_k_mu_bins"
            )
            interp_t_list[isource] = interp.interp1d(kvec,
                                                     this_tk,
                                                     kind=interp_kind,
                                                     fill_value=(this_tk[0],
                                                                 this_tk[-1]),
                                                     bounds_error=bounds_error)
...
def get_a1_a2_filters(Pks, a2_from_deltalin_minus_a1_noO2_delta_chi_div=False):
    ...
    ratio_01_11 = safe_divide(Pks[('deltalin_unsmoothed', 'delta_chi_div')][1],
                              Pks[('delta_chi_div', 'delta_chi_div')][1])
    ...
    if not a2_from_deltalin_minus_a1_noO2_delta_chi_div:
        ...
        a2tilde = (safe_divide(
            Pks[('deltalin_minus_a1_noO2_delta_chi_div',
                 'delta_chi_div_2ndorder')][1],
            Pks[('delta_chi_div_2ndorder', 'delta_chi_div_2ndorder')][1]) /
                   (1.0 - r12sq))
...
def get_general_a1_a2_filters(Pks,
                              field1=None,
                              field2=None,
                              target_field=None,
    ...
    """
    r12sq = safe_divide((Pks[(field1, field2)][1])**2,
                        (Pks[(field1, field1)][1] * Pks[(field2, field2)][1]))

    # '0' stands for target field
    ratio_01_11 = safe_divide(Pks[(target_field, field1)][1],
                              Pks[(field1, field1)][1])
    ratio_02_22 = safe_divide(Pks[(target_field, field2)][1],
                              Pks[(field2, field2)][1])
    ratio_12_11 = safe_divide(Pks[(field1, field2)][1],
                              Pks[(field1, field1)][1])
    ratio_12_22 = safe_divide(Pks[(field1, field2)][1],
                              Pks[(field2, field2)][1])
    ...
    if not a2_from_target_field_minus_a1_noO2_field1:
        ...
        a2tilde = (
            safe_divide(Pks[(target_field_minus_a1_noO2_field1, field2)][1],
                        Pks[(field2, field2)][1]) / (1.0 - r12sq))

    return a1tilde, a2tilde


def get_interp_general_a1_a2(Pks,
                             field1=None,
                             field2=None,
                             target_field=None,
                             a2_from_target_field_minus_a1_noO2_field1=False,
    ...
    a1, a2 = get_general_a1_a2_filters(
        ...
        a2_from_target_field_minus_a1_noO2_field1=
        a2_from_target_field_minus_a1_noO2_field1,
    ...
    print(
        "TODO: better implement trf fcns using manual_Pk_k_bins interpolation")
    interp_a1 = interp.interp1d(kest,
                                a1,
                                kind=interp_kind,
                                fill_value=(a1[0], a1[-1]),
                                bounds_error=bounds_error)
    interp_a2 = interp.interp1d(kest,
                                a2,
                                kind=interp_kind,
...
def get_interp_a1_a2(Pks,
                     a2_from_deltalin_minus_a1_noO2_delta_chi_div=False,
                     interp_kind=None,
                     bounds_error=False):
    # get a1, a2
    a1, a2 = get_a1_a2_filters(Pks,
                               a2_from_deltalin_minus_a1_noO2_delta_chi_div=
                               a2_from_deltalin_minus_a1_noO2_delta_chi_div)
...
def get_a1bar_noO2(Pks,
                   target_field='deltalin_unsmoothed',
                   in_field='delta_chi_div'):
    """
...
def get_interp_a1bar_noO2(Pks,
    ...
    """
    a1bar_noO2 = get_a1bar_noO2(Pks,
                                target_field=target_field,
                                in_field=in_field)
    kest = Pks[(in_field, in_field)][0]
    print("TODO: implement using manual_Pk_k_bins interpolation")
    interp_a1bar_noO2 = interp.interp1d(kest,
                                        a1bar_noO2,
                                        fill_value=(a1bar_noO2[0],
                                                    a1bar_noO2[-1]),
                                        kind=interp_kind,
                                        bounds_error=bounds_error)
