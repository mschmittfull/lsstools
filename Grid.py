...
class ComplexGrid(Grid):
    ...
    def calc_quadratic_field(self, basefield=None, quadfield=None, gridx=None,
        ...
        ----------
        basefield : string
            Linear field that we will square.
        ...
        quadfield : string
            ...
            - 'kappa2': Can get kappa2 from kappa2=-tidal_G2. Or use the extra function above.
        ...
        Returns
        -------
        Return the calculated Ngrid**3 array containing quadfield(\vk).
        """
        print("Calculate quadfield %s from %s" % (quadfield, basefield))
        assert not self.has_column('tmp_quadfield')
        ...
        if quadfield == 'growth':
            ...
            gridx.append_column(basefield, self.fft_k2x(basefield, drop_column=False))
            gridx.append_column('tmp_quadfield', gridx.G[basefield].apply(
                lambda x3vec, val: -0.5*val**2, mode='real', kind='relative'))
            gridx.drop_column(basefield)
            ...
            for idir in range(3):
                for jdir in range(idir,3):
                    ...
                    self.append_column(dijcol, 
                        self.G[basefield].apply(dij_fcn, mode='complex', kind='wavenumber'))
                    gridx.append_column(dijcol, self.fft_k2x(dijcol, drop_column=True))

                    # Add 3/2 \sum_{i,j=0..2} d_ij(\vx)d_ij(\vx) 
                    #   = 3/2 * [d_00^2+d_11^2+d_22^2 + 2*(d_01^2+d_02^2+d_12^2)]
                    if True:
                        if jdir == idir:
                            fac = 1.0
                        else:
                            fac = 2.0
                        #gridx.G['tmp_quadfield'] += 1.5 * fac * gridx.G[dijcol]**2
                        dij_x_square = gridx.G[dijcol].apply(lambda x3vec, val: val**2, mode='real', kind='relative')
                        gridx.G['tmp_quadfield'] = FieldMesh(
                            gridx.G['tmp_quadfield'].compute(mode='real') 
                            + 1.5 * fac * dij_x_square.compute(mode='real'))
                        # free memory
                        gridx.drop_column(dijcol)
                ...
                for jdir in range(idir,3):
                    ...
                    if True:
                        if jdir == idir:
                            ...
                            fac = 2.0
                        #gridx.G['tmp_quadfield'] += fac * gridx.G[dijcol]**2
                        dij_x_square = gridx.G[dijcol].apply(lambda x3vec, val: val**2, mode='real', kind='relative')
                        gridx.G['tmp_quadfield'] = FieldMesh(
                            gridx.G['tmp_quadfield'].compute(mode='real') 
            ...
            for idir in range(3):
                ...
                self.append_column('tmp_Psi_i', self.G[basefield].apply(
                    Psi_i_fcn, mode='complex', kind='wavenumber'))

                # compute nabla_i delta
                def grad_i_fcn(k3vec, val, idir=idir):
                    return -1.0j * k3vec[idir]*val

                self.append_column('tmp_grad_i', self.G[basefield].apply(
                    grad_i_fcn, mode='complex', kind='wavenumber'))

                # go to x space
                gridx.append_column('tmp_Psi_i', self.fft_k2x('tmp_Psi_i', drop_column=True))
                gridx.append_column('tmp_grad_i', self.fft_k2x('tmp_grad_i', drop_column=True))
                ...
                if idir == 0:
                    gridx.append_column(
                        'tmp_quadfield',
                        FieldMesh(
                            gridx.G['tmp_Psi_i'].compute(mode='real')
                            * gridx.G['tmp_grad_i'].compute(mode='real')) )
                else:
                    gridx.G['tmp_quadfield'] = FieldMesh(
                        gridx.G['tmp_quadfield'].compute(mode='real')
                        + (gridx.G['tmp_Psi_i'].compute(mode='real')
                           * gridx.G['tmp_grad_i'].compute(mode='real')) )
                # free memory
                gridx.drop_column('tmp_Psi_i')
                gridx.drop_column('tmp_grad_i')

        elif quadfield == 'F2':
            # work with pmesh RealGrid object
            # F2 = delta^2...
            tmp_composite_fieldx = self.calc_quadratic_field(
                basefield=basefield, quadfield='growth', gridx=gridx, return_in_k_space=False).compute(mode='real')
            # ... - shift
            tmp_composite_fieldx -= self.calc_quadratic_field(
                basefield=basefield, quadfield='shift', gridx=gridx, return_in_k_space=False).compute(mode='real')
            # ... + 2/7 tidal_G2
            tmp_composite_fieldx += 2./7. * self.calc_quadratic_field(
                basefield=basefield, quadfield='tidal_G2', gridx=gridx, return_in_k_space=False).compute(mode='real')

            # store on gridx
            if gridx.has_column('tmp_quadfield'):
                gridx.drop_column('tmp_quadfield')
            gridx.append_column('tmp_quadfield', FieldMesh(tmp_composite_fieldx))
            del tmp_composite_fieldx
                
        else:
            raise Exception("Invalid quadfield %s" % str(quadfield))
        
        # return meshsource object in x or k space
        if return_in_k_space:
            # fft quadratic field to k space and return it
            return gridx.fft_x2k('tmp_quadfield', drop_column=True)
        else:
            # return in x space
            tmp_quadfield = gridx.G['tmp_quadfield']
            gridx.drop_column('tmp_quadfield')
            return tmp_quadfield
