...
class ComplexGrid(Grid):
        ...
        for id1 in columns:
            for id2 in columns:
                if (not self.has_column(id1)) or (not self.has_column(id2)):
                    print(
                        "WARNING: Could not measure power of [%s] X [%s] b/c not on grid."
                        % (id1, id2))
                ...
                if (id1, id2) in Pkmeas:
                    ...
                    if verbose:
                        if self.comm.rank == 0:
                            ...
                            self.logger.info("Pkresult k: %s" %
                                             str(Pkresult.power['k']))
                            self.logger.info("Pkresult Nmodes: %s" %
                                             str(Pkresult.power['modes'].real))
                            self.logger.info("Pkresult P: %s" %
                                             str(Pkresult.power['power'].real))
                            self.logger.info("Pkresult attrs: %s" %
                                             str(Pkresult.attrs))
                    ...
                    if mode == '1d':
                        Pkmeas[(id1, id2)] = MeasuredPower1D(
                            nbk_binned_stat=Pkresult,
                            info=info,
                            info_id1=self.column_infos[id1],
                            info_id2=self.column_infos[id2])
                    elif mode == '2d':
                        Pkmeas[(id1, id2)] = MeasuredPower2D(
                            nbk_binned_stat=Pkresult,
                            info=info,
                            info_id1=self.column_infos[id1],
                            info_id2=self.column_infos[id2])
    ...
    def calc_kappa2(self, field_column, gridx=None):
        ...
        gridx.append_column(field_column,
                            self.fft_k2x(field_column, drop_column=False))
        # start with field**2(x)
        gridx.append_column(
            'tmp_kappa2', gridx.G[field_column].apply(lambda x3vec, val: val**2,
                                                      mode='real',
                                                      kind='relative'))
        ...
        for idir in range(3):
            for jdir in range(idir, 3):
                ...
                self.append_column(
                    dijcol, self.G[field_column].apply(dij_fcn,
                                                       mode='complex',
                                                       kind='wavenumber'))
                gridx.append_column(dijcol,
                                    self.fft_k2x(dijcol, drop_column=True))
                ...
                if True:
                    ...
                    dij_x_square = gridx.G[dijcol].apply(lambda x3vec, val: val
                                                         **2,
                                                         mode='real',
                                                         kind='relative')
                    gridx.G['tmp_kappa2'] = FieldMesh(
                        gridx.G['tmp_kappa2'].compute(mode='real') -
                        fac * dij_x_square.compute(mode='real'))
                    # free memory
    ...
    def calc_quadratic_field(self,
        ...
        if smoothing_of_base_field is not None:
            ...
            return nbkit03_utils.calc_quadratic_field(
                base_field_mesh=self.G['TMP_FIELD'], quadfield=quadfield)

        else:
            # no smoothing needed
            return nbkit03_utils.calc_quadratic_field(
                base_field_mesh=self.G[basefield], quadfield=quadfield)
    ...
    def store_smoothed_gridx(self,
        ...
        if replace_nan is False:
            self.append_column(tmpcol,
                               FieldMesh(self.G[col].compute(mode='complex')))
        else:

            def replacer(k3vec, val):
                return np.where(
                    np.isnan(val),
                    np.zeros(val.shape, dtype=val.dtype) + replace_nan, val)

            self.append_column(
                tmpcol, self.G[col].apply(replacer,
                                          mode='complex',
                                          kind='wavenumber'))
        ...
        helper_gridx.save_to_bigfile(out_fname,
                                     columns=[tmpcol],
                                     new_dataset_for_each_column=True)
        if plot:
            helper_gridx.plot_slice(tmpcol,
                                    'liveslice_%s_R%s.png' % (col, str(R)))
    ...
    def compute_orthogonalized_fields(self,
        ...
        for iortho in range(1, N_ortho_iter + 1):
            print("\n\nOrthogonalize fields for trf fcns, iteration %d of %d" %
                  (iortho, N_ortho_iter))
            ...
            if orth_method == 'CholeskyDecomp':
                ...
                inv_sqrt_Sii_vec = np.zeros(
                    (Nfields,
                     Nk)) + np.nan  # normalization D_ij=delta_ij S_ii^{-1/2}
                for ik in range(Nk):
                    if False:
                        ...
                        Sii = np.diag(Smat[:, :, ik])
                        inv_sqrt_Sii_vec[:, ik] = np.where(
                            Sii > 0., 1. / np.sqrt(Sii), 0 * Sii)
                        if not np.all(inv_sqrt_Sii_vec[:, ik] > 0.):
                            # one of the vectors has 0 length, so do not rotate
                            print(
                                "Warning: not rotating k, Sii, inv_sqrt_Sii_vec:",
                ...
                for ifield in range(Nfields):
                    for jfield in range(Nfields):
                        Mrotmat[ifield, jfield, :] = (
                            inv_Lmat[ifield, jfield, :] *
                            inv_sqrt_Sii_vec[jfield, :] /
                            (inv_Lmat[ifield, ifield, :] *
                             inv_sqrt_Sii_vec[ifield, :]))
                ...
                for ifield, field1 in enumerate(all_fields):
                    for jfield, field2 in enumerate(all_fields):
                        if field1.startswith(
                                non_orth_prefix) or field2.startswith(
                                    non_orth_prefix):
                            if jfield == ifield:
                                Mrotmat[ifield, jfield, :] = 0 * Mrotmat[
                                    ifield, jfield, :] + 1.0
                            else:
                                Mrotmat[ifield, jfield, :] = 0 * Mrotmat[
                                    ifield, jfield, :]
                ...
                for ifield in range(Nfields):
                    for jfield in range(Nfields):
                        if False:
                            ...
                            raise Exception(
                                "please use manual_Pk_k_bins interpolation")
                            interp_Mrotmat[ifield][jfield] = interp.interp1d(
                                kvec,
                                Mrotmat[ifield, jfield, :],
                                kind=
                                'nearest',  # 'nearest' gives better orthogonalization than 'linear'
                                fill_value=(Mrotmat[ifield, jfield, 0],
                                            Mrotmat[ifield, jfield, -1]),
                                bounds_error=False)
                        else:
                            # use manual k binning interp: gives much better orthogonalization (10^-5 or better)
                            interp_Mrotmat[ifield][
                                jfield] = interpolation_utils.interp1d_manual_k_binning(
                                    kvec,
                                    Mrotmat[ifield, jfield, :],
                ...
                for ifield in range(Nfields):
                    ...
                    if old_all_fields[ifield].startswith(orth_prefix):
                        ...
                        self.append_column(
                            ...
                            FieldMesh(0.0 *
                                      self.G['%s^%d_%d' %
                                             (orth_prefix, iortho - 1,
                                              ifield)].compute(mode='complex')))
                        ...
                        for jfield in range(0, ifield + 1):
                            ...
                            if interp_kind == 'manual_Pk_k_bins':
                                ...
                                def to_add_filter(k3vec, val):
                                    ...
                                    return interp_Mrotmat[ifield][jfield](
                                        absk) * val
                                ...
                                def to_add_filter(k3vec, val):
                                    ...
                                    with np.errstate(invalid='ignore',
                                                     divide='ignore'):
                                        mu = sum(k3vec[i] * RSD_los[i]
                                                 for i in range(3)) / absk
                                    return interp_Mrotmat[ifield][jfield](
                                        absk, mu) * val

                            to_add = self.G['%s^%d_%d' %
                                            (orth_prefix, iortho - 1,
                                             jfield)].apply(to_add_filter,
                                                            mode='complex',
                                                            kind='wavenumber')
                            self.G[ofield] = FieldMesh(
                                self.G[ofield].compute(mode='complex') +
                        ...
                        self.append_column(
                            ofield,
                            self.G['%s^%d_%d' %
                                   (non_orth_prefix, iortho - 1, ifield)])
                    else:
                ...
                # original fields.
                raise Exception(
                    "Not implemented any more in parallel version of the code.")
            ...
            if delete_original_fields:
                for ifield in range(Nfields):
                    if old_all_fields[ifield].startswith(orth_prefix):
                        self.drop_column('%s^%d_%d' %
                                         (orth_prefix, iortho - 1, ifield))
                    elif old_all_fields[ifield].startswith(non_orth_prefix):
                        self.drop_column('%s^%d_%d' %
                                         (non_orth_prefix, iortho - 1, ifield))
        ...
        if test_orthogonality:
            ...
            for i1, c1 in enumerate(cols):
                for i2, c2 in enumerate(cols):
                    if i2 > i1:
                        continue
                    rcc = Pktest[(c1, c2)].P / (Pktest[(c1, c1)].P *
                                                Pktest[(c2, c2)].P)**0.5
    ...
    def convert_to_weighted_uniform_catalog(self,
        ...
        if helper_gridx is None:
            print("nan k:",
                  np.where(np.isnan(self.G[col].compute(mode='complex'))))
            ...
            helper_gridx = RealGrid(meshsource=my_meshsource_x,
                                    ...
                                    boxsize=self.boxsize)
            if np.isnan(
                    get_cstat(helper_gridx.G[col].compute(mode='real'), 'max')):
                raise Exception("err 1: gridx is nan after fft")

        else:
            helper_gridx.append_column(col, self.fft_k2x(col,
                                                         drop_column=False))
        ...
        return helper_gridx.convert_to_weighted_uniform_catalog(
            col=col,
            uni_cat_Nptcles_per_dim=uni_cat_Nptcles_per_dim,
...
def main():
    ...
    gridx = RealGrid(grid_array=gridx_array,
                     column='delta_x',
                     Ngrid=Ngrid,
                     boxsize=L)
    ...
    ## Apply a k filter
    # multiply by k_z
    test_k = np.einsum('c,abc->abc', gridk.k_component_1d,
    ...
    for direction in [0, 1, 2]:
        ...
        gridk.append_column(
            col,
            np.einsum('%s,abc,abc->abc' % index, 1j * gridk.k_component_1d,
                      (gridk.G3d('INV_ABSK'))**2, gridk.G3d('delta_k')))
