...
def generate_sources_and_get_interp_filters_minimizing_sqerror(
    ...
    for source in quadratic_sources:
        ...
        gridk.append_column(
            sqcol,
            gridk.calc_quadratic_field(basefield='%s_smoothed' %
                                       field_to_smoothen_and_square,
                                       quadfield=source))
    ...
    if field_to_smoothen_and_square2 is not None:
        gridk.append_column('%s_smoothed' % field_to_smoothen_and_square2,
    ...
    print("sources_for_trf_fcn, save_bestfit_field:", sources_for_trf_fcn,
          save_bestfit_field)
            ...
            if not gridk.has_column(s):
                ...
                if s == 'deltalin+deltalin_F2':
                    ...
                    gridk.append_column(
                        ...
                        FieldMesh(gridk.G['delta_h_WEIGHT_M1'].compute(
                            mode='complex') -
                                  gridk.G['delta_h'].compute(mode='complex')))
                    if ('delta_h_WEIGHT_M1' not in sources) and (
                            'delta_h_WEIGHT_M1' != target):
    ...
    for isource, source in enumerate(sources):
        ...
        if source in fixed_linear_sources:
            pass
              ...
              not None) and (source in target_spec.linear_target_contris):
            pass
        else:
    ...
    if target_spec is not None:
        ...
        if target_spec.minimization_objective in [
            ...
            if target_spec.minimization_objective == '(T*target-T*sources)^2/(T*target)^2':
                if N_ortho_iter == 0:
                    raise Exception(
                        "Must orthogonalize all sources (maths assumes this). Please set N_ortho_iter=1."
                    )
                if non_orth_linear_sources not in [None, []]:
                    raise Exception(
                        "Must orthogonalize all sources (maths assumes this).")
                if fixed_linear_sources not in [None, []]:
                    ...
                    print(
                        "Warning: Target weights are not optimal if model has fixed_linear sources, but proceed anyways."
                    )
            ...
            if target_spec.target_norm['type'].startswith('MatchPower'):
                ...
                if target_spec.target_norm[
                        'Pk_to_match_id1'] != target_spec.target_norm[
                            'Pk_to_match_id2']:
            ...
            if target_spec.minimization_objective in [
                    '(target0+T*other_targets-T*sources)^2',
                    '(T*target-T*sources)^2/(T*sources)^2'
            ]:
                ...
                if len(fixed_sources) > 0:
                    ...
                    for fs in fixed_sources:
                        TMP_target_minus_fixed_sources = '%s_MINUS_[%s]' % (
                            TMP_target_minus_fixed_sources, fs)
                    print("TMP_target_minus_fixed_sources=%s" %
                          TMP_target_minus_fixed_sources)
                    # compute target-fixed sources on grid
                    gridk.append_column(TMP_target_minus_fixed_sources,
                                        gridk.G[orth_target_contris[0]])
                    for fs in fixed_sources:
                        #gridk.G[TMP_target_minus_fixed_sources] -= gridk.G[fs]
                        gridk.G[TMP_target_minus_fixed_sources] = FieldMesh(
                            gridk.G[TMP_target_minus_fixed_sources].compute(
                                mode='complex') -
                ...
                if Pk_1d_2d_mode == '1d':
                    TMP_interp_tuple_osources = transfer_functions.highlevel_get_interp_filters_minimizing_sqerror(
                        sources=TMP_osources,
                        target=TMP_target_minus_fixed_sources,
                        Pk=Pkmeas,
                ...
                if target_spec.target_norm['type'] in [
                        'alpha0=1', 'MatchPower', 'MatchPowerAndLowKLimit'
                ]:
                    if Pk_1d_2d_mode != '1d':
                        raise Exception('Not implemented')
                    # Start with alpha0=1, alpha1=T_0(k), alpha2=T_1(k), etc
                    my_interp_alpha_opt = (np.zeros(
                        (N_target_contris,))).tolist()
                    ...
                    for itc in range(1, N_target_contris):
                        ...
                        my_interp_alpha_opt[itc] = (
                            lambda myk: -TMP_interp_tuple_osources[itc - 1](myk)
                        )
                    ...
                    if target_spec.target_norm['type'] == 'alpha0=1':
                    ...
                    elif target_spec.target_norm['type'] in [
                            'MatchPower', 'MatchPowerAndLowKLimit'
                    ]:
                        ...
                        if False:
                            ...
                            gridk.append_column(
                                'TMP_FIELD_FOR_NORM',
                                0.0 * gridk.G[orth_target_contris[0]].compute(
                                    mode='complex'))
                            for itc, tc_itc in enumerate(orth_target_contris):
                                ...
                                def mult_by_alpha(k3vec, val, itc=itc):
                                    absk = np.sqrt(sum(
                                        ki**2
                                        for ki in k3vec))  # absk on the mesh
                                ...
                                gridk.G['TMP_FIELD_FOR_NORM'] = FieldMesh(
                                    gridk.G['TMP_FIELD_FOR_NORM'].compute(
                                        mode='complex') +
                            ...
                            for itc, tc in enumerate(orth_target_contris):
                                tmp_Ptarget += Pkmeas[(tc, tc)][1] * (
                                    my_interp_alpha_opt[itc](kvec))**2
                        # normfac = sqrt(P_wanted / P_target)
                        tmp_normfac = np.sqrt(Pkmeas[
                            (target_spec.target_norm['Pk_to_match_id1'],
                        ...
                        if target_spec.target_norm[
                            ...
                            my_Pratio = (Pkmeas[(
                                target_spec.
                                target_norm['LowK_Pnorm_to_match_id1'],
                                target_spec.
                                target_norm['LowK_Pnorm_to_match_id2']
                            )][1] / Pkmeas[(
                                target_spec.target_norm['Pk_to_match_id1'],
                                target_spec.target_norm['Pk_to_match_id2'])][1])
                            print("MatchPowerAndLowKLimit Pratio:", my_Pratio)
                            k_indices = target_spec.target_norm[
                                'LowK_k_bin_indices']
                            lowk_const = np.mean(my_Pratio[k_indices])
                            print(
                                "MatchPowerAndLowKLimit lowk_const (expect 1/b1):",
                                lowk_const)
                        ...
                        if Pk_1d_2d_mode != '1d':
                            ...
                            for itc in range(N_target_contris):
                                interp_alpha_opt[itc] = (
                                    lambda myk: interp_tmp_normfac(
                                        myk) * my_interp_alpha_opt[itc](myk))

                else:
                    raise Exception("Invalid target_norm %s" %
                                    str(target_spec.target_norm['type']))
                ...
                for itc, tc_itc in enumerate(orth_target_contris):
                    for jtc, tc_jtc in enumerate(orth_target_contris):
                        ...
                        for mu in range(Nsources):
                            r_itc_mu = Pkmeas[
                                (tc_itc, osources[mu])][1] / np.sqrt(
                                    Pkmeas[(tc_itc, tc_itc)][1] *
                                    Pkmeas[(osources[mu], osources[mu])][1])
                            r_jtc_mu = Pkmeas[
                                (tc_jtc, osources[mu])][1] / np.sqrt(
                                    Pkmeas[(tc_jtc, tc_jtc)][1] *
                                    Pkmeas[(osources[mu], osources[mu])][1])
                ...
                Amat_Qmat = np.zeros(
                    (N_target_contris, N_target_contris, Nk)) + np.nan
                alpha_opt = np.zeros((N_target_contris, Nk)) + np.nan
                for ik in range(Nk):
                    ...
                    if target_spec.target_norm['type'] == 'alpha0=1':
                        ...
                        Pkfix = Pkmeas[(
                            target_spec.target_norm['Pk_to_match_id1'],
                            target_spec.target_norm['Pk_to_match_id2'])][1][ik]
                        sumQsqPii = 0.0
                        for tmp_itarget in range(N_target_contris):
                            sumQsqPii += (
                                Amat_Qmat[tmp_itarget, 0, ik]**2 * Pkmeas[
                                    (orth_target_contris[tmp_itarget],
                                     orth_target_contris[tmp_itarget])][1][ik])
                        ...
                        target_normfac = np.sqrt(Pkfix / sumQsqPii) * np.sign(
                            Amat_Qmat[0, 0, 0])
                    else:
                        raise Exception("Invalid target_norm %s" %
                                        str(target_spec.target_norm))
                    # compute alpha
                ...
                for itc in range(N_target_contris):
                    ...
                    if Pk_1d_2d_mode != '1d':
                        ...
                        interp_alpha_opt[
                            itc] = interpolation_utils.interp1d_manual_k_binning(
                                kvec,
                                alpha_opt[itc, :],
                                #kind='manual_Pk_k_bins',
                                kind=interp_kind,
                                fill_value=(alpha_opt[itc, 0],
                                            alpha_opt[itc, -1]),
                                bounds_error=False,
                                Ngrid=gridk.Ngrid,
                                L=gridk.boxsize,
            ...
            gridk.append_column(
                target_spec.save_bestfit_target_field,
                FieldMesh(
                    0.0 *
                    gridk.G[orth_target_contris[0]].compute(mode='complex')),
            ...
            for itc, tc_itc in enumerate(orth_target_contris):
                ...
                to_add = gridk.G[tc_itc].apply(mult_by_alpha,
                                               mode='complex',
                                               kind='wavenumber')
                gridk.G[target_spec.save_bestfit_target_field] = FieldMesh(
                    gridk.G[target_spec.save_bestfit_target_field].compute(
                        mode='complex') + to_add.compute(mode='complex'))
                del mult_by_alpha, to_add
    ...
    if len(fixed_sources) > 0:
        if target_spec is not None:
            ...
            if target_spec.minimization_objective == '(T*target-T*sources)^2/(T*sources)^2':
                raise Exception(
                    "Fixed sources not implemented if minimizing (T*target-T*sources)^2/(T*sources)^2"
                )
        # get id of target-fixed_sources
        target_minus_fixed_sources = '[%s]' % target
        for fs in fixed_sources:
            target_minus_fixed_sources = '%s_MINUS_[%s]' % (
                target_minus_fixed_sources, fs)
    ...
    if save_bestfit_field is not None:
        ...
        for counter, s in enumerate(osources):
            ...
            to_add = gridk.G[s].apply(multiply_me,
                                      mode='complex',
                                      kind='wavenumber')
    ...
    for tmpcol in gridk.G.keys():
        ...
        if tmpcol == 'ABSK' or tmpcol == save_bestfit_field:
            continue
              ...
              not None) and (tmpcol == target_spec.save_bestfit_target_field):
            continue
        else:
            if gridk.has_column(tmpcol):
                gridk.drop_column(tmpcol)
    ...
    for osource, interp_otf in zip(osources, interp_tuple_osources):
        if Pk_1d_2d_mode == '1d':
            ...
            trf_fcns_orth_fields[target][osource] = [(kvec, muvec),
                                                     interp_otf(kvec, muvec)]
    ...
    if orth_method == 'CholeskyDecomp':
        if N_ortho_iter in [0, None]:
            ...
            for osource in trf_fcns_orth_fields[target].keys():
                trf_results['trf_fcns_orig_fields'][target][
                    initial_source_of_osource[osource]] = (
        ...
        if ortho_rot_matrix_sources is None:
            ...
            for isource in range(Nsources):
                ...
                for jsource in range(Nsources):
                    Tk_sources[isource, :] += ortho_rot_matrix_sources[
                        isource, jsource, :] * Tk_osources[jsource, :]
            ...
            for isource in range(Nsources):
                raise Exception("please use manual_Pk_k_bins interpolation")
                interp_tuple.append(
                    interp.interp1d(kvec,
                                    Tk_sources[isource, :],
                                    kind=interp_kind,
                                    bounds_error=bounds_error,
                                    fill_value=(Tk_sources[isource, 0],
                                                Tk_sources[isource, -1])))
